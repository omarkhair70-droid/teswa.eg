...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (devicesError) {
      console.error("Failed querying push devices", { code: devicesError.code, message: devicesError.message, notificationId: record.id });
    ...
    if (!expoResponse.ok) {
      console.error("Expo push API request failed", { status: expoResponse.status, body: await expoResponse.text(), notificationId: record.id });
    ...
    if (invalidTokenIndexes.length > 0) {
      ...
      if (disableError) {
        console.error("Failed disabling invalid push tokens", { message: disableError.message, notificationId: record.id, invalidTokenCount: invalidTokens.length });
      }
    }
