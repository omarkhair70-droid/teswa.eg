...
begin
  ...
  if notification_type in ('deal_message_received', 'deal_voice_message_received') then
    ...
    if v_message_sender_id is distinct from v_sender_id then
      return;
    end if;
    ...
    insert into public.notifications (
      ...
      v_recipient_id,
      v_sender_id,
      case when v_message_type = 'voice' then 'deal_voice_message_received' else 'deal_message_received' end,
      'رسالة جديدة على تِسوى',
      case
