...
-- Additive by design: Stream stays untouched until native parity is proven.
...
for select to authenticated using (
  exists (select 1 from public.direct_conversations c where c.id=direct_message_attachments.conversation_id and auth.uid() in (c.participant_a,c.participant_b))
);
...
for select to authenticated using (
  exists (select 1 from public.direct_conversations c where c.id=direct_message_reactions.conversation_id and auth.uid() in (c.participant_a,c.participant_b))
);
...
for select to authenticated using (
  exists (select 1 from public.direct_conversations c where c.id=direct_typing_state.conversation_id and auth.uid() in (c.participant_a,c.participant_b))
);
...
create policy "direct_chat_media_delete_sender" on storage.objects
...
begin
  ...
  if p_reply_to_message_id is not null and not exists (
    select 1 from public.direct_messages dm where dm.id=p_reply_to_message_id and dm.conversation_id=p_conversation_id and dm.deleted_at is null
  ) then return query select false,'الرسالة التي ترد عليها غير متاحة.',null::uuid,null::uuid,null::timestamptz; return; end if;
...
language sql security definer set search_path=public as $$
  select
    ...
    case when m.deleted_at is not null then '[]'::jsonb else coalesce((
      select jsonb_agg(jsonb_build_object(
        'id',a.id,'kind',a.kind,'storagePath',a.storage_path,'fileName',a.file_name,'mimeType',a.mime_type,
        'sizeBytes',a.size_bytes,'durationMs',a.duration_ms,'width',a.width,'height',a.height
  ...
  where m.conversation_id=p_conversation_id
    ...
    and exists (select 1 from public.direct_conversations c where c.id=p_conversation_id and auth.uid() in (c.participant_a,c.participant_b))
