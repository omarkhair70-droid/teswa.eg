...
import { buildCityPulseSignals } from '@/lib/city-pulse-signals';
...
export function CityPulseSection({ location, snapshot, loadingLocation, loadingPulse, error, bootstrapping, cacheNotice, onActivate, onRefresh, onHide, onRetry }: CityPulseSectionProps) {
  if (bootstrapping) return <View style={styles.card}><AppText weight="bold">نستعيد نبض مدينتك...</AppText><AppText muted>تِسوى يراجع آخر مشهد محلي محفوظ.</AppText></View>;
  ...
  if (location && loadingPulse && !snapshot) return <View style={styles.card}><AppText weight="bold">نبض تِسوى حولك</AppText><AppText muted>{`نقرأ نبض: ${location.label}`}</AppText><AppText muted>جارٍ جمع الحركة القريبة...</AppText>{cacheNotice ? <AppText muted>{cacheNotice}</AppText> : null}</View>;
  ...
  return <View style={styles.card}><View style={styles.row}><View><AppText weight="bold">نبض تِسوى حولك</AppText><AppText muted>{`نقرأ نبض: ${location.label}`}</AppText></View><View style={styles.actions}><Pressable onPress={onRefresh}><AppText style={styles.link}>تحديث النبض</AppText></Pressable><Pressable onPress={onHide}><AppText style={styles.linkMuted}>إخفاء</AppText></Pressable></View></View>
    ...
    <View style={styles.signalsBlock}><AppText weight="semibold">نبض المدينة الآن</AppText>{pulseSignals.map((signal) => <View key={signal.id} style={[styles.signalRow, signal.tone === 'movement' ? styles.signalMovement : null, signal.tone === 'stories' ? styles.signalStories : null, signal.tone === 'people' ? styles.signalPeople : null, signal.tone === 'quiet' ? styles.signalQuiet : null]}><View style={[styles.signalMarker, signal.tone === 'movement' ? styles.signalMarkerMovement : null, signal.tone === 'stories' ? styles.signalMarkerStories : null, signal.tone === 'people' ? styles.signalMarkerPeople : null, signal.tone === 'quiet' ? styles.signalMarkerQuiet : null]} /><View style={styles.signalContent}><AppText weight="semibold">{signal.title}</AppText><AppText muted>{signal.body}</AppText></View></View>)}</View>
...
const styles = StyleSheet.create({ card: { borderWidth: 1, borderColor: colors.border, borderRadius: radii.xl, backgroundColor: colors.surface, padding: spacing.md, gap: spacing.sm }, row: { flexDirection: 'row', justifyContent: 'space-between', gap: spacing.sm }, actions: { alignItems: 'flex-end', gap: spacing.xs }, link: { color: colors.primary }, linkMuted: { color: colors.textMuted }, cacheNotice: { borderWidth: 1, borderColor: colors.border, borderRadius: radii.md, backgroundColor: colors.background, padding: spacing.sm }, signalsBlock: { borderWidth: 1, borderColor: colors.border, borderRadius: radii.lg, backgroundColor: colors.background, padding: spacing.sm, gap: spacing.xs }, signalRow: { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.xs, borderWidth: 1, borderColor: colors.border, borderRadius: radii.md, padding: spacing.xs }, signalContent: { flex: 1, gap: 2 }, signalMarker: { width: 4, borderRadius: radii.sm, alignSelf: 'stretch' }, signalMovement: { backgroundColor: colors.primarySoft }, signalStories: { backgroundColor: colors.surface }, signalPeople: { backgroundColor: colors.background }, signalQuiet: { backgroundColor: colors.background }, signalMarkerMovement: { backgroundColor: colors.primary }, signalMarkerStories: { backgroundColor: colors.accent }, signalMarkerPeople: { backgroundColor: colors.border }, signalMarkerQuiet: { backgroundColor: colors.textMuted }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.xs }, chip: { backgroundColor: colors.primarySoft, borderRadius: radii.md, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs }, chipText: { color: colors.primary, fontSize: 12 }, section: { gap: spacing.xs }, rail: { gap: spacing.sm, paddingRight: spacing.md }, itemCard: { width: 180, borderWidth: 1, borderColor: colors.border, borderRadius: radii.lg, padding: spacing.sm, gap: spacing.xs, backgroundColor: colors.white }, personCard: { width: 120, gap: spacing.xs, alignItems: 'center' }, avatar: { width: 56, height: 56, borderRadius: radii.round }, avatarFallback: { width: 56, height: 56, borderRadius: radii.round, backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center' }, thumb: { width: '100%', height: 100, borderRadius: radii.md, backgroundColor: colors.background }, thumbFallback: { alignItems: 'center', justifyContent: 'center' }, micro: { color: colors.primary, fontSize: 12 }, badge: { color: colors.accent, fontSize: 12 }, error: { color: '#B42318' }, errorInline: { borderWidth: 1, borderColor: '#B42318', borderRadius: radii.md, padding: spacing.sm, gap: spacing.xs } });
