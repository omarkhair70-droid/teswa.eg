...
type Props = {
  sort: DolabSortMode;
  status: DolabStatusFilter;
  onSortChange: (value: DolabSortMode) => void;
  onStatusChange: (value: DolabStatusFilter) => void;
};

export function DolabFilterChips(props: Props) {
  return (
    <>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.row}>
        {SORTS.map((item) => (
          <Chip
            key={item.key}
            label={item.label}
            active={props.sort === item.key}
            onPress={() => props.onSortChange(item.key)}
          />
        ))}
      </ScrollView>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.row}>
        {FILTERS.map((item) => (
          <Chip
            key={item.key}
            label={item.label}
            active={props.status === item.key}
            onPress={() => props.onStatusChange(item.key)}
          />
        ))}
      </ScrollView>
    </>
