...
export function DolabVaultHero() {
  ...
  useEffect(() => {
    ...
    glowLoop.start(); driftLoop.start(); ringLoop.start();
    return () => {
      glowLoop.stop(); driftLoop.stop(); ringLoop.stop();
      glow.stopAnimation(); drift.stopAnimation(); ring.stopAnimation();
    };
  }, [drift, glow, ring]);
  ...
  return (
    <LinearGradient colors={['#FFF8EE', '#F4EDE4', '#F2F7F6']} style={styles.hero}>
      ...
      <Animated.View style={[styles.floatingChip, { transform:[{ translateY: drift.interpolate({inputRange:[0,1], outputRange:[0,-8]}) }] }]}>
        <Ionicons name="lock-closed-outline" size={14} color={colors.primary} /><AppText style={styles.chipText}>خاص</AppText>
      </Animated.View>
      <Animated.View style={[styles.floatingChipSecondary, { transform:[{ translateY: drift.interpolate({inputRange:[0,1], outputRange:[-2,6]}) }] }]}>
        <Ionicons name="sparkles-outline" size={14} color={colors.accent} /><AppText style={styles.chipText}>حيّ</AppText>
      </Animated.View>
