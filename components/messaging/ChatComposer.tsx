      ...
      <View style={styles.wrap}>
        ...
        <View style={styles.recordingShell}>
          <Pressable
            ...
            hitSlop={8}
            style={({ pressed }) => [styles.recordingAction, pressed && styles.pressed]}
            disabled={recording.busy || recording.sending}
          ...
          <View style={styles.recordingBody}>
            ...
            <View style={styles.waveRow} accessibilityElementsHidden>
              {WAVE.map((height, index) => (
                <View
                  key={`composer-wave-${index}`}
                  style={[
                    styles.waveBar,
                    { height, opacity: index % 4 === 0 ? 0.55 : index % 3 === 0 ? 0.72 : 0.92 },
                  ]}
                />
              ))}
            </View>
          ...
          <Pressable
            ...
            disabled={recording.busy || recording.sending}
            onPress={recording.onSend}
          >
            {recording.busy || recording.sending ? (
        ...
        {onPressAttachment ? (
          ...
          >
            <Ionicons
              name="add-circle"
              size={28}
              color={disabled || attachmentDisabled ? colors.border : colors.primary}
            />
          </Pressable>
        ) : null}
        ...
        {hasSendablePayload ? (
          <Pressable
            ...
            style={({ pressed }) => [
              ...
              !canSend && styles.primaryActionDisabled,
              pressed && canSend && styles.primaryPressed,
            ]}
          >
            {sending ? (
              <ActivityIndicator size="small" color={colors.background} />
          ...
          >
            <Ionicons
              name="mic"
              size={21}
              color={disabled || voiceDisabled ? colors.border : colors.primary}
            />
          </Pressable>
        ) : (
...
const styles = StyleSheet.create({
  ...
  replyAccent: { width: 3, alignSelf: 'stretch', borderRadius: 2, backgroundColor: colors.primary },
  replyCopy: { flex: 1, gap: 1 },
  replyLabel: { fontSize: 12, color: colors.primary, textAlign: 'right' },
  replyText: { fontSize: 13, textAlign: 'right' },
  replyClose: { width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center' },
  recordingShell: {
    minHeight: 58,
    flexDirection: 'row-reverse',
    alignItems: 'center',
  ...
  recordingAction: { width: 42, height: 46, borderRadius: 23, alignItems: 'center', justifyContent: 'center' },
  recordingBody: {
    flex: 1,
    minHeight: 50,
    justifyContent: 'center',
  ...
  recordingMeta: { flexDirection: 'row-reverse', alignItems: 'center', gap: 7 },
  liveDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: colors.danger },
  recordingLabel: { fontSize: 12, color: colors.text },
  recordingTime: { marginLeft: 'auto', fontVariant: ['tabular-nums'], fontSize: 12 },
  waveRow: { height: 23, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 2 },
