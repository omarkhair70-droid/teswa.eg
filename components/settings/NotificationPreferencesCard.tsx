...
const ITEMS: Array<{ key: ToggleKey; title: string; description: string }> = [
  ...
  { key: 'quietHoursEnabled', title: 'وضع الهدوء', description: 'تقليل التنبيهات غير العاجلة في وقت الراحة.' },
];
export function NotificationPreferencesCard({ preferences, loading, savingKey, onToggle }: Props) {
  return <AppCard><View style={styles.header}><AppText weight="semibold">إعدادات الإشعارات</AppText></View>{ITEMS.map((item) => <View key={item.key} style={styles.row}><View style={styles.rowText}><AppText weight="semibold">{item.title}</AppText><AppText muted style={styles.description}>{item.description}</AppText></View><Switch value={preferences[item.key]} disabled={loading || savingKey === item.key} onValueChange={(value) => onToggle(item.key, value)} trackColor={{ false: colors.border, true: colors.primarySoft }} thumbColor={preferences[item.key] ? colors.primary : '#f4f3f4'} /></View>)}<AppText muted style={styles.note}>يتم حفظ تفضيلات الهدوء حالياً كإعدادات فقط، وسيتم ربط التنفيذ الكامل لاحقاً.</AppText></AppCard>;
}
