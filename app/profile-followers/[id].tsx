...
export default function FollowersScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    setError(null);
    const { data, error: rpcError } = await (await import('@/lib/supabase/client')).supabase.rpc('get_profile_followers', { p_profile_user_id: id, p_limit: 50 });
    if (rpcError) {
      setError('تعذر تحميل قائمة المتابعين حالياً. حاول مرة أخرى.');
      setRows([]);
      setLoading(false);
      return;
    }
  ...
  return <AppScreen scrollable><AppCard><AppText weight="semibold">المتابعون</AppText>{loading ? <AppText muted>جاري التحميل...</AppText> : null}{!loading && error ? <View style={styles.group}><AppText muted>{error}</AppText><AppButton label="إعادة المحاولة" variant="neutral" onPress={() => void load()} /></View> : null}{!loading && !error && rows.length === 0 ? <EmptyState title="لا يوجد متابعون بعد." description="عندما يبدأ أشخاص بمتابعة هذا الملف ستظهر أسماؤهم هنا." /> : null}{!loading && !error ? rows.map((p) => <Row key={p.profile_id} profile={p} currentUserId={user?.id ?? ''} onRefresh={load} />) : null}</AppCard></AppScreen>;
}
