...
export default function AddScreen() {
  ...
  const pickImages = async () => {
    ...
    setAssets(result.assets.slice(0, 4));
  };
  ...
  const next = () => { const e = validateCurrentStep(); setError(e); if (!e) setStep((s) => Math.min(s + 1, 5)); };
  ...
  return <AppScreen scrollable><View style={styles.header}><AppText weight='bold' style={styles.title}>نشر عنصر جديد</AppText><AppText muted>الخطوة {step + 1} من 6 — {steps[step]}</AppText></View>
    ...
    {step===0 && <AppCard><View style={styles.gap}><AppButton label='اختيار الصور من المعرض' onPress={pickImages} /><View style={styles.row}>{assets.map((a,i)=><Image key={a.uri} source={{uri:a.uri}} style={styles.preview} />)}</View><AppText muted>اختر من 1 إلى 4 صور.</AppText></View></AppCard>}
...
const styles = StyleSheet.create({ header:{gap:spacing.xs}, title:{fontSize:24}, gap:{gap:spacing.sm}, row:{flexDirection:'row',gap:spacing.sm, flexWrap:'wrap'}, rowWrap:{flexDirection:'row', flexWrap:'wrap', gap:spacing.sm}, preview:{width:72,height:72,borderRadius:10, backgroundColor:colors.border}, chip:{paddingHorizontal:12,paddingVertical:8,borderWidth:1,borderColor:colors.border,borderRadius:999}, chipSelected:{borderColor:colors.primary, backgroundColor:colors.primarySoft}, actions:{flexDirection:'row',gap:spacing.sm}, error:{color:colors.primary} });
