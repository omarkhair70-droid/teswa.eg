...
export default function FollowingScreen() {
  ...
  const load = useCallback(async () => {
    if (!id) return;
    setLoading(true);
    const { data } = await (await import('@/lib/supabase/client')).supabase.rpc('get_profile_following', { p_profile_user_id: id, p_limit: 50 });
    setRows((data ?? []) as FollowProfileRow[]);
    setLoading(false);
  }, [id]);
  useEffect(() => { void load(); }, [load]);

  return <AppScreen scrollable><AppCard><AppText weight="semibold">يتابع</AppText>{loading ? <AppText muted>جاري التحميل...</AppText> : null}{rows.map((p) => <Row key={p.profile_id} profile={p} currentUserId={user?.id ?? ''} onRefresh={load} />)}</AppCard></AppScreen>;
}
