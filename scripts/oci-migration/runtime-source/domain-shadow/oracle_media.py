    ...
    def __init__(self, bucket='teswa-media'):
        try:
            ...
            self.client = oci.object_storage.ObjectStorageClient(
                {},
                signer=signer,
                service_endpoint=f'https://objectstorage.{signer.region}.oraclecloud.com'
            )
        ...
        if method == 'POST' and parsed.path == '/v1/media/signed-url':
            ...
            try:
                purpose, key, _content_type, _size, physical = object_input(
                    body, user_id, False, allow_non_owner=purpose_value in ('deal_voice', 'direct_voice', 'contextual_voice', 'item_video', 'story_media'))
            ...
            if found is None:
                raise ApiError(404, 'media_not_found')
            signed_url = self.store().par(physical, 'ObjectRead', expires, 'read')
            return 200, {'signedUrl': signed_url,
