...
WORK="$(mktemp -d)"; ARCHIVE="$WORK/notifications-analytics-core.tar.gz"; CONTENT="$WORK/content.json"; TARGET="$WORK/target.json"; UPLOADED=false
...
for f in runtime-notifications-analytics-core.sql verify-runtime-notifications-analytics-core.sql notifications-analytics-core-guest-apply.sh; do [ -f "$ROOT/scripts/oci-migration/$f" ] || { echo "notifications_analytics_operator=FAIL reason=missing_$f" >&2; exit 2; }; done
mkdir -p "$WORK/stage"
cp "$ROOT/scripts/oci-migration/runtime-notifications-analytics-core.sql" "$ROOT/scripts/oci-migration/verify-runtime-notifications-analytics-core.sql" "$ROOT/scripts/oci-migration/notifications-analytics-core-guest-apply.sh" "$WORK/stage/"
chmod 700 "$WORK/stage/notifications-analytics-core-guest-apply.sh"
tar -C "$WORK/stage" -czf "$ARCHIVE" runtime-notifications-analytics-core.sql verify-runtime-notifications-analytics-core.sql notifications-analytics-core-guest-apply.sh
SHA="$(sha256sum "$ARCHIVE"|awk '{print $1}')"; OBJECT="lane4-rehearsal/notifications-analytics-core/$STAMP-$SHA.tar.gz"
...
[ -n "$INSTANCE_ID" ] && [ "$INSTANCE_ID" != null ] || { echo 'notifications_analytics_operator=FAIL reason=core_instance_not_found'; exit 3; }
...
[ "$STATE" = RUNNING ] || { echo 'notifications_analytics_operator=FAIL reason=target_not_running'; exit 4; }
oci os object put --bucket-name "$BUCKET" --name "$OBJECT" --file "$ARCHIVE" --force >/dev/null; UPLOADED=true

echo 'TESWA LANE 4 NOTIFICATIONS ANALYTICS CORE OPERATOR'
echo 'database=teswa_rehearsal'
echo 'function_signatures=11'
echo 'policies=7'
...
set -Eeuo pipefail
OBJ='__OBJECT__'; SHA='__SHA__'; D="$(mktemp -d /var/tmp/teswa-notif-analytics-XXXXXX)"; trap 'rm -rf "$D"' EXIT
sudo -n true || { echo 'notifications_analytics_operator=FAIL reason=no_sudo'; exit 10; }
...
PY
printf '%s  %s\n' "$SHA" "$D/a.tgz"|sha256sum -c - >/dev/null || { echo 'notifications_analytics_operator=FAIL reason=artifact_sha'; exit 12; }
tar -xzf "$D/a.tgz" -C "$D"; chmod 700 "$D/notifications-analytics-core-guest-apply.sh"; bash "$D/notifications-analytics-core-guest-apply.sh" "$D"
GUEST
)"
SCRIPT_TEXT="${SCRIPT_TEXT//__OBJECT__/$OBJECT}"; SCRIPT_TEXT="${SCRIPT_TEXT//__SHA__/$SHA}"; BYTES="$(printf '%s' "$SCRIPT_TEXT"|wc -c|tr -d ' ')"; echo "guest_script_bytes=$BYTES"; [ "$BYTES" -le 4096 ] || { echo 'notifications_analytics_operator=FAIL reason=run_command_text_limit'; exit 5; }
...
echo 'notifications_analytics_artifact_cleanup=PASS'; echo 'notifications_analytics_operator_cloudshell=PASS'
