...
WORK="$(mktemp -d)"; ARCHIVE="$WORK/deals-core.tar.gz"; CONTENT="$WORK/content.json"; TARGET="$WORK/target.json"; UPLOADED=false
...
for f in runtime-deals-core.sql verify-runtime-deals-core.sql deals-core-guest-apply.sh; do [ -f "$ROOT/scripts/oci-migration/$f" ] || { echo "deals_operator=FAIL reason=missing_$f" >&2; exit 2; }; done
mkdir -p "$WORK/stage"
cp "$ROOT/scripts/oci-migration/runtime-deals-core.sql" "$ROOT/scripts/oci-migration/verify-runtime-deals-core.sql" "$ROOT/scripts/oci-migration/deals-core-guest-apply.sh" "$WORK/stage/"
chmod 700 "$WORK/stage/deals-core-guest-apply.sh"
tar -C "$WORK/stage" -czf "$ARCHIVE" runtime-deals-core.sql verify-runtime-deals-core.sql deals-core-guest-apply.sh
SHA="$(sha256sum "$ARCHIVE"|awk '{print $1}')"; OBJECT="lane4-rehearsal/deals-core/$STAMP-$SHA.tar.gz"
...
[ -n "$INSTANCE_ID" ] && [ "$INSTANCE_ID" != null ] || { echo 'deals_operator=FAIL reason=core_instance_not_found'; exit 3; }
...
[ "$STATE" = RUNNING ] || { echo 'deals_operator=FAIL reason=target_not_running'; exit 4; }
oci os object put --bucket-name "$BUCKET" --name "$OBJECT" --file "$ARCHIVE" --force >/dev/null; UPLOADED=true

echo 'TESWA LANE 4 DEALS CORE OPERATOR'
...
set -Eeuo pipefail
OBJ='__OBJECT__'; SHA='__SHA__'; D="$(mktemp -d /var/tmp/teswa-deals-core-XXXXXX)"; trap 'rm -rf "$D"' EXIT
sudo -n true || { echo 'deals_operator=FAIL reason=no_sudo'; exit 10; }
...
PY
printf '%s  %s\n' "$SHA" "$D/a.tgz"|sha256sum -c - >/dev/null || { echo 'deals_operator=FAIL reason=artifact_sha'; exit 12; }
tar -xzf "$D/a.tgz" -C "$D"; chmod 700 "$D/deals-core-guest-apply.sh"; bash "$D/deals-core-guest-apply.sh" "$D"
GUEST
)"
SCRIPT_TEXT="${SCRIPT_TEXT//__OBJECT__/$OBJECT}"; SCRIPT_TEXT="${SCRIPT_TEXT//__SHA__/$SHA}"; BYTES="$(printf '%s' "$SCRIPT_TEXT"|wc -c|tr -d ' ')"; echo "guest_script_bytes=$BYTES"; [ "$BYTES" -le 4096 ] || { echo 'deals_operator=FAIL reason=run_command_text_limit'; exit 5; }
...
PY
CID="$(oci instance-agent command create --compartment-id "$COMPARTMENT" --content "file://$CONTENT" --target "file://$TARGET" --timeout-in-seconds 300 --display-name 'teswa-lane4-deals-core' --query 'data.id' --raw-output)"; echo "command_id=$CID"
...
echo 'deals_artifact_cleanup=PASS'; echo 'deals_operator_cloudshell=PASS'
