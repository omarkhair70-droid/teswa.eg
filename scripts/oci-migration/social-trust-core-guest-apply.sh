...
[ -f "$STAGE/runtime-social-trust-core.sql" ] && [ -f "$STAGE/verify-runtime-social-trust-core.sql" ] || { echo 'social_trust_operator=FAIL reason=missing_sql'; exit 13; }
...
BAD_FN="$(sudo -u postgres "$P" -d "$DB" -Atqc "SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname IN ('follow_user','unfollow_user','get_user_block_state','get_user_follow_state','get_my_badges','get_my_trust_metrics','is_admin_user','refresh_my_badges','report_user','report_story','report_direct_message') AND pg_get_functiondef(p.oid) LIKE '%auth.uid()%'")"
