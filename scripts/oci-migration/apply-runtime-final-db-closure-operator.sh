...
[ "$STATE" = RUNNING ] || { echo 'final_db_closure_operator=FAIL reason=target_not_running'; exit 4; }
oci os object put --bucket-name "$BUCKET" --name "$OBJECT" --file "$ARCHIVE" --force >/dev/null; UPLOADED=true

echo 'TESWA LANE 4 FINAL DATABASE RUNTIME CLOSURE OPERATOR'
...
set -Eeuo pipefail
OBJ='__OBJECT__'; SHA='__SHA__'; D="$(mktemp -d /var/tmp/teswa-final-db-XXXXXX)"; trap 'rm -rf "$D"' EXIT
sudo -n true || { echo 'final_db_closure_operator=FAIL reason=no_sudo'; exit 10; }
...
PY
printf '%s  %s\n' "$SHA" "$D/a.tgz"|sha256sum -c - >/dev/null || { echo 'final_db_closure_operator=FAIL reason=artifact_sha'; exit 12; }
tar -xzf "$D/a.tgz" -C "$D"; chmod 700 "$D/final-db-closure-guest-apply.sh"; bash "$D/final-db-closure-guest-apply.sh" "$D"
GUEST
)"
SCRIPT_TEXT="${SCRIPT_TEXT//__OBJECT__/$OBJECT}"; SCRIPT_TEXT="${SCRIPT_TEXT//__SHA__/$SHA}"; BYTES="$(printf '%s' "$SCRIPT_TEXT"|wc -c|tr -d ' ')"; echo "guest_script_bytes=$BYTES"; [ "$BYTES" -le 4096 ] || { echo 'final_db_closure_operator=FAIL reason=run_command_text_limit'; exit 5; }
...
PY
CID="$(oci instance-agent command create --compartment-id "$COMPARTMENT" --content "file://$CONTENT" --target "file://$TARGET" --timeout-in-seconds 300 --display-name 'teswa-lane4-final-db-closure' --query 'data.id' --raw-output)"; echo "command_id=$CID"
...
echo 'final_db_closure_artifact_cleanup=PASS'; echo 'final_db_closure_operator_cloudshell=PASS'
