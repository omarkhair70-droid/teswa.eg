...
WORK="$(mktemp -d)"
ARCHIVE="$WORK/push-worker.tar.gz"
...
for f in push-shadow-worker.py push-shadow-guest-deploy.sh; do
  [ -f "$ROOT/scripts/oci-migration/$f" ] || { echo "push_worker_operator=FAIL reason=missing_$f" >&2; exit 2; }
done
mkdir -p "$WORK/stage"
cp "$ROOT/scripts/oci-migration/push-shadow-worker.py" "$WORK/stage/worker.py"
cp "$ROOT/scripts/oci-migration/push-shadow-guest-deploy.sh" "$WORK/stage/guest-deploy.sh"
chmod 700 "$WORK/stage/guest-deploy.sh"
tar -C "$WORK/stage" -czf "$ARCHIVE" worker.py guest-deploy.sh
SHA="$(sha256sum "$ARCHIVE" | awk '{print $1}')"
OBJECT="lane4-rehearsal/push-worker/$STAMP-$SHA.tar.gz"
...
[ -n "$INSTANCE_ID" ] && [ "$INSTANCE_ID" != null ] || { echo 'push_worker_operator=FAIL reason=core_instance_not_found' >&2; exit 3; }
...
[ "$STATE" = RUNNING ] || { echo 'push_worker_operator=FAIL reason=target_not_running' >&2; exit 4; }
oci os object put --bucket-name "$BUCKET" --name "$OBJECT" --file "$ARCHIVE" --force >/dev/null
UPLOADED=true

echo 'TESWA LANE 4 PUSH WORKER OPERATOR'
...
DIR="$(mktemp -d /var/tmp/teswa-push-worker-XXXXXX)"; trap 'rm -rf "$DIR"' EXIT
sudo -n true || { echo 'push_worker_operator=FAIL reason=no_passwordless_sudo'; exit 10; }
[ "$(hostname -s)" = core01 ] || { echo 'push_worker_operator=FAIL reason=unexpected_guest_hostname'; exit 11; }
...
PY
printf '%s  %s\n' "$SHA" "$DIR/bundle.tar.gz" | sha256sum -c - >/dev/null || { echo 'push_worker_operator=FAIL reason=artifact_sha_mismatch'; exit 12; }
tar -xzf "$DIR/bundle.tar.gz" -C "$DIR"
chmod 700 "$DIR/guest-deploy.sh"
bash "$DIR/guest-deploy.sh" "$DIR"
echo 'push_worker_operator=PASS'
GUEST
)"
...
[ "$BYTES" -le 4096 ] || { echo 'push_worker_operator=FAIL reason=run_command_text_limit' >&2; exit 5; }
...
PY
CID="$(oci instance-agent command create --compartment-id "$COMPARTMENT" --content "file://$CONTENT_FILE" --target "file://$TARGET_FILE" --timeout-in-seconds 300 --display-name 'teswa-lane4-push-shadow-worker' --query 'data.id' --raw-output)"
echo "command_id=$CID"
while true; do
...
UPLOADED=false
echo 'push_worker_artifact_cleanup=PASS'
echo 'push_worker_operator_cloudshell=PASS'
