...
(async () => {
  ...
  const adapter = mod.exports.createOracleMarketplaceReadAdapter({ request: async (input) => {
    ...
    if (input.path.endsWith('/video-presence')) return { ok: true, status: 200, data: { values: {video:true} } };
    if (input.path.endsWith('/video')) return { ok: true, status: 200, data: { item: null } };
    if (input.path.endsWith('/count-since')) return { ok: true, status: 200, data: { count: 2 } };
  ...
  if((await adapter.getItemVideoPresence(['video'])).get('video')!==true) throw new Error('video presence failed');
  if((await adapter.getItemVideoMetadata('none'))!==null) throw new Error('video metadata failed');
  if((await adapter.countMarketplaceItemsSince('2026-09-08T00:00:00Z'))!==2) throw new Error('count failed');
  if((await adapter.listNearbyFeed({latitude:30,longitude:31})).hasMore!==false) throw new Error('nearby failed');
  if((await adapter.listRecentItemVideoDiscovery(5)).length!==0||(await adapter.listMovingItems(5)).length!==0||(await adapter.listPulseItemTeasers(5)).length!==0||(await adapter.listItemStoryDiscovery(5)).length!==0)throw new Error('discovery reads failed');
