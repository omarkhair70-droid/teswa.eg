...
function logGoogleSignInDiagnostic(flow: GoogleSignInDiagnosticFlow, reason: GoogleSignInDiagnosticReason) {
...
export async function signInWithGoogleNative(options?: GoogleNativeSignInOptions): Promise<NativeGoogleSignInResult> {
  ...
  try {
    ...
    emitGoogleNativeStep({ flow: 'native_step', step: 'native_success', hasIdToken: true, implementation: GOOGLE_NATIVE_AUTH_IMPLEMENTATION }, options);
    ...
    emitGoogleNativeStep({ flow: 'native_step', step: 'native_catch', code, message, implementation: GOOGLE_NATIVE_AUTH_IMPLEMENTATION }, options);
