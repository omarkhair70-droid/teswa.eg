...
const DIRECT_MESSAGE_CACHE_KEY_PREFIX = 'direct-chat:native:v2:';
const DIRECT_MESSAGE_CACHE_INDEX_KEY = 'direct-chat:native:v2:index';
...
function sanitizeAttachments(value: unknown): CachedDirectMessageAttachment[] | undefined {
  ...
  const attachments = value
    .map((attachment): CachedDirectMessageAttachment => ({
  ...
  return messages
    .map((message): CachedDirectMessage | null => {
      ...
      return {
        ...
        ownReactions: ownReactions?.length ? ownReactions : undefined,
...
function writeIndex(mmkv: TeswaMmkvStorage, entries: DirectMessageCacheIndexEntry[]) {
...
export function readDirectMessageCache(conversationIdInput: string): DirectMessageCacheSnapshot | null {
  ...
  try {
    ...
    if (!updatedAtMs || !messages.length) return null;
...
export function writeDirectMessageCache(conversationIdInput: string, messagesInput: CachedDirectMessage[]) {
  const conversationId = conversationIdInput.trim();
  const mmkv = getStorage();
  ...
  try {
    if (!messages.length) mmkv.remove(cacheKey(conversationId));
    else mmkv.set(cacheKey(conversationId), JSON.stringify({ conversationId, updatedAtMs, messages }));

    const existing = readIndex(mmkv).filter((entry) => entry.conversationId !== conversationId);
    const next = messages.length ? [{ conversationId, updatedAtMs }, ...existing] : existing;
    writeIndex(mmkv, next.slice(0, DIRECT_MESSAGE_CACHE_MAX_CONVERSATIONS));
    pruneDirectMessageCache();
  } catch {}
}

export function pruneDirectMessageCache(nowMs = Date.now()) {
  const mmkv = getStorage();
  if (!mmkv) return;
  try {
    ...
    sorted.forEach((entry) => {
      ...
      if (expired || overLimit) mmkv.remove(cacheKey(entry.conversationId));
