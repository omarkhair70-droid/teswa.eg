...
const SESSION_SKEW_SECONDS = 15;
const listeners = new Set<(event: AuthStateEvent, session: TeswaAuthSession | null) => void>();
let refreshInFlight: Promise<TeswaAuthSession | null> | null = null;
...
function mapApiFailure(status: number, body: JsonObject): TeswaResult<never, AuthFailureReason> {
  ...
  if (status >= 500) return failure('network', 'Oracle authentication is temporarily unavailable.');
...
function isUsable(session: TeswaAuthSession): boolean {
  return session.expiresAt !== null && session.expiresAt > Math.floor(Date.now() / 1000) + SESSION_SKEW_SECONDS;
}
...
export function createOracleAuthAdapter(options: OracleAuthAdapterOptions = {}): AuthContract {
  ...
  const refreshSession = async (refreshToken: string): Promise<TeswaAuthSession | null> => {
    ...
    const operation = (async () => {
      ...
      if (generation !== sessionGeneration) return readOracleAuthSession();
      if (!response.ok || response.status < 200 || response.status >= 300) {
        if (response.ok && response.status === 401) await clearSession();
        return null;
      }
  ...
  return {
    getSession,
    async getCurrentUser() { return (await getSession())?.user ?? null; },
    ...
    async signUp(input) {
      ...
      if (!response.ok) return failure('network', response.message, response.cause);
      if (response.status < 200 || response.status >= 300) return mapApiFailure(response.status, response.body);
      return success({ user: mapUser(response.body.user), session: null });
    },
    ...
    async signInWithExternalIdToken(input) {
      ...
      const response = await requestJson(baseUrl, '/v1/auth/google', {
        method: 'POST', body: { id_token: input.idToken },
      });
      if (!response.ok) return failure('network', response.message, response.cause);
      if (response.status < 200 || response.status >= 300) return mapApiFailure(response.status, response.body);
      const session = mapSession(response.body);
    ...
    async signOut() {
      ...
      const response = await requestJson(baseUrl, '/v1/auth/logout', {
        method: 'POST', accessToken: local.accessToken,
      });
      if (!response.ok) return failure('network', response.message, response.cause);
      if (response.status >= 500) return failure('unknown', 'Oracle auth logout failed.');
      await clearSession();
      return { ok: true, data: undefined };
    },
    subscribeToAuthState(listener) {
      listeners.add(listener);
      Promise.resolve().then(getSession).then((session) => listener('initial', session)).catch(() => listener('initial', null));
