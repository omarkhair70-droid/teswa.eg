...
export function getGoogleNativeAuthModuleInfo(): GoogleNativeAuthModuleInfo {
  return { moduleVersion: GOOGLE_NATIVE_AUTH_MODULE_VERSION, implementation: GOOGLE_NATIVE_AUTH_IMPLEMENTATION, platform: Platform.OS, hasSignInFunction: typeof signInWithGoogleNative === 'function' };
}

export function logGoogleSignInDiagnostic(flow: 'native' | 'browser_fallback', reason: string) {
...
export async function signInWithGoogleNative(options?: GoogleNativeSignInOptions): Promise<NativeGoogleSignInResult> {
  options?.onStep?.({ flow: 'native_step', step: 'native_helper_entered', reason: 'non_android', platform: Platform.OS, implementation: GOOGLE_NATIVE_AUTH_IMPLEMENTATION, moduleVersion: GOOGLE_NATIVE_AUTH_MODULE_VERSION });
