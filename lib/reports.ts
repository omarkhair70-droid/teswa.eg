...
export async function submitDealReport(input: { dealId: string; currentUserId: string; reason: ReportReason; details?: string }) { if (!ALLOWED_REASONS.includes(input.reason)) return { ok: false as const, message: 'سبب البلاغ غير صالح.' }; const c = await fetchDealReportContext(input.dealId, input.currentUserId); if (!c.ok) return c; return reportUser({ reportedUserId: c.context.reportedUser.id, reason: input.reason, details: input.details }); }
export async function submitStoryReport(input: { storyId: string; currentUserId: string; reason: ReportReason; details?: string }) { if (!ALLOWED_REASONS.includes(input.reason)) return { ok: false as const, message: 'سبب البلاغ غير صالح.' }; const c = await fetchStoryReportContext(input.storyId, input.currentUserId); if (!c.ok) return c; const { error } = await supabase.from('reports').insert({ reporter_id: input.currentUserId, reported_user_id: c.context.author.id, story_id: input.storyId, reason: input.reason, details: input.details?.trim() || null, status: 'open' }); if (error) return mapRpcError(error); return { ok: true as const, message: SUCCESS_MESSAGE }; }
