...
export async function markContextualThreadReadFromMobile(conversationId: string): Promise<void> {
  ...
  const { error } = await supabase.rpc('mark_contextual_thread_read', { p_conversation_id: normalizedConversationId });
  if (error && __DEV__) console.warn('[contextual-conversations] mark read failed', error);
}
...
export async function fetchUnreadContextualMessagesCount(): Promise<number> {
  ...
  if (error) {
    if (__DEV__) console.warn('[contextual-conversations] unread count failed', error);
    return 0;
  }
...
export async function sendStoryReplyFromMobile(input: {
  ...
  const { data, error } = await supabase.rpc('create_story_reply_thread', { p_story_id: storyId, p_body: body });
  const row = Array.isArray(data) ? data[0] : null;
  if (error || !row?.conversation_id || !row?.message_id) {
    if (__DEV__) console.warn('[contextual-conversations] create story reply failed', error, data);
...
export async function fetchContextualConversationSummariesForUser(userId: string): Promise<ContextualConversationSummary[]> {
  ...
  const participantIds = Array.from(new Set(conversations.flatMap((c) => [c.starter_id, c.recipient_id])));
  ...
  const messagesByConversation = new Map<string, (typeof messagesRes.data extends Array<infer T> ? T : never)[]>();
  ...
  return conversations
    .map((conversation): ContextualConversationSummary => {
      ...
      const unreadCount = conversationMessages.filter((m) => m.sender_id !== normalizedUserId && (!lastReadAt || m.created_at > lastReadAt)).length;
      return {
        ...
        latestMessage: latest
          ...
          : null,
        unreadCount,
        lastActivityAt: latest?.created_at ?? conversation.updated_at ?? conversation.created_at ?? new Date(0).toISOString(),
      };
    })
...
export async function fetchContextualThreadById(input: {
  ...
  if (conversation.starter_id !== currentUserId && conversation.recipient_id !== currentUserId) return { ok: false, reason: 'unauthorized' };
  ...
  const profilesById = new Map((profilesRes.data ?? []).map((p) => [p.id, p]));
  const otherId = currentUserId === conversation.starter_id ? conversation.recipient_id : conversation.starter_id;
  ...
  return {
    ...
    thread: {
      ...
      messages: (messagesRes.data ?? []).map((m) => ({ id: m.id, conversationId: m.conversation_id, senderId: m.sender_id, body: m.body, createdAt: m.created_at })),
    },
  };
}
...
export async function sendContextualMessageFromMobile(input: {
  ...
  if (body.length > 800) return { ok: false, reason: 'invalid_body', message: 'الرسالة طويلة زيادة عن الحد (800 حرف).' };
  ...
  if (error || !data) {
    if (__DEV__) console.warn('[contextual-conversations] send message failed', error);
    return { ok: false, reason: 'send_failed', message: 'تعذر إرسال الرسالة حالياً.' };
  }

  return { ok: true, message: { id: data.id, conversationId: data.conversation_id, senderId: data.sender_id, body: data.body, createdAt: data.created_at } };
}
