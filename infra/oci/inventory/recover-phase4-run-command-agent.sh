...
if [ "${TESWA_ALLOW_CORE_REBOOT:-}" != "YES" ]; then
  echo "Refusing mutation: set TESWA_ALLOW_CORE_REBOOT=YES to reboot only teswa-core-01." >&2
  exit 2
fi
...
INSTANCE_JSON="$(oci compute instance list   --compartment-id "$COMPARTMENT"   --display-name "teswa-core-01"   --all   --output json)"
...
if len(rows)!=1:
    raise SystemExit("expected exactly one active teswa-core-01")
print(rows[0]["id"])
')"
...
echo "TESWA PHASE 4 CORE RUN COMMAND RECOVERY"
