...
oci_capture EDGE_ID edge_instance \
...
oci_capture CORE_ID core_instance \
  oci compute instance list \
    --compartment-id "$COMPARTMENT" \
...
for pair in "edge:$EDGE_ID" "core:$CORE_ID"; do
  label="${pair%%:*}"; value="${pair#*:}"
  if [ -z "$value" ] || [ "$value" = "null" ] || [ "$value" = "None" ]; then
    echo "phase8_caddy_core_verify=FAIL reason=missing_running_instance target=$label" >&2
    exit 3
  fi
done
...
oci_capture EDGE_PRIVATE_IP edge_private_ip \
  oci compute instance list-vnics \
    --instance-id "$EDGE_ID" \
    --query 'data[0]."private-ip"' \
    --raw-output

if [ -z "$EDGE_PRIVATE_IP" ] || [ "$EDGE_PRIVATE_IP" = "null" ] || [ "$EDGE_PRIVATE_IP" = "None" ]; then
  echo "phase8_caddy_core_verify=FAIL reason=edge_private_ip_missing" >&2
  exit 4
fi

echo "probe_target=$EDGE_PRIVATE_IP:8080"

oci_capture EDGE_NSG_ID edge_nsg \
...
oci_capture APP_NSG_ID app_nsg \
  oci network nsg list \
    --compartment-id "$COMPARTMENT" \
    --all \
...
for pair in "edge_nsg:$EDGE_NSG_ID" "app_nsg:$APP_NSG_ID"; do
  label="${pair%%:*}"; value="${pair#*:}"
  if [ -z "$value" ] || [ "$value" = "null" ] || [ "$value" = "None" ]; then
    echo "phase8_caddy_core_verify=FAIL reason=missing_nsg target=$label" >&2
    exit 5
  fi
done

oci_capture RULES edge_nsg_rules \
  oci network nsg rules list \
    --network-security-group-id "$EDGE_NSG_ID" \
    --all \
    --output json
...
try:
    ...
    print(f"phase8_caddy_core_verify=FAIL reason=edge_nsg_rules_invalid_json detail={type(exc).__name__}")
    raise SystemExit(20)
private_ok=False
public_8080=False
for r in rows:
...
if not private_ok:
    print("phase8_caddy_core_verify=FAIL reason=private_8080_rule_missing")
    raise SystemExit(21)
if public_8080:
    print("phase8_caddy_core_verify=FAIL reason=public_8080_detected")
    raise SystemExit(22)
PY
...
oci_capture COMMAND_ID core_probe_create \
...
elapsed=0
while true; do
  if ! oci_capture EXEC_JSON core_probe_execution \
