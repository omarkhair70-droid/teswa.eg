...
printf '%s\n' "TESWA PHASE 8B EDGE-CORE PRIVATE ROUTING PREFLIGHT"
printf '%s\n' "mutation=none"
...
for value in "$EDGE_NSG_ID" "$APP_NSG_ID"; do
  [ -n "$value" ] && [ "$value" != "null" ] && [ "$value" != "None" ] || {
    echo "phase8b_preflight=FAIL reason=missing_nsg" >&2
    exit 5
  }
done
...
if ss -ltnH | awk '$4 ~ /(^|:)3100$/ && $4 != "127.0.0.1:3100" {bad=1} END {exit bad ? 0 : 1}'; then
  echo "core_runtime=FAIL reason=unexpected_api_listener"; exit 15
fi
if ss -ltnH | awk '$4 ~ /(^|:)3200$/ && $4 != "127.0.0.1:3200" {bad=1} END {exit bad ? 0 : 1}'; then
  echo "core_runtime=FAIL reason=unexpected_realtime_listener"; exit 16
fi
fw_3100=false
fw_3200=false
