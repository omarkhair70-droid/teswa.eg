...
"$TF" show -json "$PLAN" >/tmp/teswa-phase4-bastion-canonical-ingress-plan.json

python3 - /tmp/teswa-phase4-bastion-canonical-ingress-plan.json <<'PY'
import json,sys
p=json.load(open(sys.argv[1],encoding="utf-8"))
expected_updates={"oci_core_security_list.admin_bastion_egress[0]"}
...
print("PHASE4 CANONICAL BASTION INGRESS PLAN GUARD")
print("adds=%d" % len(adds))
print("updates=%d" % len(updates))
print("destroys=%d" % len(destroys))
for x in sorted(updates): print("update="+x)

if adds or updates!=expected_updates or destroys or bad:
    print("phase4_bastion_ingress_plan_guard=FAIL")
    print("unexpected_adds=%s" % sorted(adds))
...
print("phase4_bastion_ingress_plan_guard=PASS")
PY
